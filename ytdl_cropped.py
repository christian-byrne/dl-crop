...
def format_from_webapp(videos, filetype=".mp4"):
    ...
    videos["url"] = str(videos["url"][0])
    videos["title"] = str(videos["title"][0])
    videos["start"] = int(float(videos["start"][0]))
    videos["end"] = int(float(videos["end"][0]))

    videos["file name"] = \
        videos["title"].replace("+", "_").replace("%20", "_").lower()
    for character in illegal_chars:
        videos["file name"] = \
            videos["file name"].replace(character, "_") 
    # Add iterator to file name if file name already exists in cd
    while not duplicate_names(videos["file name"]):
        videos["file name"] += "_1"
    videos["file name"] += filetype if filetype[0] == "." else "." + filetype 

    # when getting entire list of dicts for all selections
...
def dl_crop(custom_settings=False, from_webapp=False):
    ...
    else: videos = format_from_webapp(from_webapp)
    ...
    if from_webapp: crop( [videos], settings)
    else: crop(videos, settings)
